...
class ChessiumServer:
    def __init__(self, host="0.0.0.0", port=5555):
        self.host = host
        self.port = port
        self.rooms = {}         # room_id -> host_socket
        self.match_queue = []   # list of socket objects waiting for matchmaking
    ...
    def start(self):
        ...
        server.listen(5)
        print(f"[SERVER] Matchmaker Server started on {self.host}:{self.port}")
    ...
    def handle_client(self, conn, addr):
        try:
                ...
                if msg_type == "CREATE":
                    ...
                    with self.lock:
                        if room_id in self.rooms:
                            host_conn = self.rooms[room_id]
                            conn.sendall(json.dumps({"type": "START"}).encode('utf-8'))
                            host_conn.sendall(json.dumps({"type": "START"}).encode('utf-8'))
                        else:
                            conn.sendall(json.dumps({"type": "JOIN_RESPONSE", "status": "fail"}).encode('utf-8'))

                elif msg_type == "JOIN_QUEUE":
                    with self.lock:
                        if conn not in self.match_queue:
                            self.match_queue.append(conn)
                        
                        if len(self.match_queue) >= 2:
                            player1 = self.match_queue.pop(0)
                            player2 = self.match_queue.pop(0)
                            
                            match_room_id = f"auto_match_{random.randint(1000, 9999)}"
                            self.rooms[match_room_id] = player1

                            player1.sendall(json.dumps({"type": "QUEUE_MATCH", "role": "host", "room_id": match_room_id}).encode('utf-8'))
                            player2.sendall(json.dumps({"type": "QUEUE_MATCH", "role": "join", "room_id": match_room_id}).encode('utf-8'))

                elif msg_type == "LEAVE_QUEUE":
                    with self.lock:
                        if conn in self.match_queue:
                            self.match_queue.remove(conn)
            ...
            with self.lock:
                if conn in self.match_queue:
                    self.match_queue.remove(conn)
                for rid, s in list(self.rooms.items()):
                    if s == conn:
                        del self.rooms[rid]
            conn.close()
...
def connect_multiplayer():
    host_rect = pygame.Rect(40, 270, 180, 110)
    join_rect = pygame.Rect(250, 270, 180, 110)
    queue_rect = pygame.Rect(460, 270, 180, 110) # Added 3rd Button: Match Queue
    ...
    while selecting:
        ...
        pygame.draw.rect(screen, (119, 149, 86), queue_rect, border_radius=12)
        pygame.draw.rect(screen, (80, 80, 80), back_rect, border_radius=8)
        
        h_text = pygame.font.SysFont("arial", 22, bold=True).render("Create Room", True, (245, 245, 245))
        j_text = pygame.font.SysFont("arial", 22, bold=True).render("Join Room", True, (245, 245, 245))
        q_text = pygame.font.SysFont("arial", 22, bold=True).render("Match Queue", True, (245, 245, 245))
        ...
        screen.blit(q_text, q_text.get_rect(center=queue_rect.center))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if host_rect.collidepoint(pos):
                    ...
                    role = "join"
                    selecting = False
                elif queue_rect.collidepoint(pos):
                    role = "queue"
                    selecting = False
    ...
    if role == "queue":
        # Queue Connection Loop
        waiting_in_queue = True
        cancel_queue_rect = pygame.Rect((WIDTH - 200) // 2, 400, 200, 45)
        try:
            sock.sendall(json.dumps({"type": "JOIN_QUEUE"}).encode('utf-8'))
        except Exception:
            try: sock.close()
            except: pass
            return "BACK_TO_MENU"
        ...
        while waiting_in_queue:
            screen.fill((30, 31, 34))
            info = pygame.font.SysFont("arial", 24).render("Searching for an opponent...", True, (180, 180, 180))
            anim_dots = "." * (int(time.time() * 2) % 4)
            dots_surf = pygame.font.SysFont("arial", 32, bold=True).render(anim_dots, True, (119, 149, 86))
            
            screen.blit(info, info.get_rect(center=(WIDTH // 2, 230)))
            screen.blit(dots_surf, dots_surf.get_rect(center=(WIDTH // 2, 280)))
            
            pygame.draw.rect(screen, (220, 80, 80), cancel_queue_rect, border_radius=8)
            c_text = pygame.font.SysFont("arial", 18, bold=True).render("Cancel Search", True, (255, 255, 255))
            screen.blit(c_text, c_text.get_rect(center=cancel_queue_rect.center))
            ...
            try:
                ...
                if data:
                    packet = json.loads(data.decode('utf-8'))
                    if packet.get("type") == "QUEUE_MATCH":
                        global player
                        assigned_role = packet.get("role")
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        pos = pygame.mouse.get_pos()
                        if cancel_queue_rect.collidepoint(pos):
                            try:
                                sock.sendall(json.dumps({"type": "LEAVE_QUEUE"}).encode('utf-8'))
                                sock.close()
                            except:
                                pass
                            return "BACK_TO_MENU"
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--server":
        server = ChessiumServer(host="0.0.0.0", port=5555)
        server.start()
    else:
        main()
