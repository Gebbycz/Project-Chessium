...
class ChessGame:
    def __init__(self, mode="bot"):
        ...
        self.my_turn = True if mode == "bot" else False
    ...
    def resign(self):
        ...
        if self.mode == "p2p" and self.conn:
            ...
            self.update_ratings(outcome="loss")
            print("Bot wins! Local rating adjusted.")
    ...
    def offer_draw(self):
        ...
        if self.mode == "bot":
            import random
            bot_accepts = random.choice([True, False])
            if bot_accepts:
                print("\nBot accepted the draw offer.")
                self.game_active = False
                self.update_ratings(outcome="draw")
            else:
                print("\nBot declined the draw offer. Play continues!")
        
        elif self.mode == "p2p" and self.conn:
            print("\nSending draw offer to opponent...")
            packet = {"type": "DRAW_OFFER"}
            try:
                self.conn.sendall(json.dumps(packet).encode('utf-8'))
            except Exception:
                print("Failed to send draw offer.")

    def handle_incoming_draw_offer(self):
        print("\n[NOTIFICATION] Opponent has offered a draw.")
        response = input("Accept draw? (yes/no): ").strip().lower()
        ...
        if response == "yes":
            ...
            packet = {"type": "DRAW_RESPONSE", "accepted": False}
            self.conn.sendall(json.dumps(packet).encode('utf-8'))
            print("You declined the draw. Play continues.")

    def update_ratings(self, outcome):
        print(f"Match over. Processing Elo adjustment for: {outcome.upper()}")
...
pygame.display.set_caption("Chessium Beta-test.1")
...
def select_game_mode():
    selecting = True
    bot_rect = pygame.Rect(30, 270, 180, 100)
    local_rect = pygame.Rect(240, 270, 180, 100)
    p2p_rect = pygame.Rect(450, 270, 180, 100)
    while selecting:
        ...
        pygame.draw.rect(screen, (100, 180, 240), bot_rect, border_radius=10)
        ...
        bot_text = pygame.font.SysFont("arial", 22, bold=True).render("Play Bot", True, (20, 20, 20))
...
def connect_multiplayer():
    ...
    while selecting:
        ...
        h_text = pygame.font.SysFont("arial", 30, bold=True).render("Host Match", True, (20, 20, 20))
        j_text = pygame.font.SysFont("arial", 30, bold=True).render("Join Match", True, (20, 20, 20))
    ...
    if role == "host":
        ...
        while waiting:
            screen.fill((50, 50, 50))
            info = pygame.font.SysFont("arial", 24).render(f"Waiting for your opponent on port 5555", True, (255, 255, 255))
            ip_info = pygame.font.SysFont("arial", 28, bold=True).render(f"Your IP: {local_ip}", True, (100, 240, 120))
            screen.blit(info, info.get_rect(center=(WIDTH // 2, 250)))
            screen.blit(ip_info, ip_info.get_rect(center=(WIDTH // 2, 320)))
            pygame.display.flip()
        ...
        while connecting:
            screen.fill((50, 50, 50))
            lbl = pygame.font.SysFont("arial", 24).render("Enter Host's IP Address:", True, (255, 255, 255))
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_BACKSPACE:
                        ...
                        try:
                            ...
                            error_msg = "Connection failed! Check IP."
                    else:
...
def handle_remote_move(move_uci):
    move = chess.Move.from_uci(move_uci)
    if move in board.legal_moves:
        play_move_sound(board, move)
        board.push(move)
        p2p_game.my_turn = True

if game_mode == "p2p":
    p2p_game.start_p2p_listener(handle_remote_move)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if pos[0] < BOARD_SIZE and pos[1] < BOARD_SIZE:
                ...
                if game_mode == "p2p" and not p2p_game.my_turn:
                    continue
                ...
                if selected is None:
                    ...
                    if move in board.legal_moves:
                        ...
                        if game_mode == "p2p":
                            p2p_game.my_turn = False
                            packet = {"type": "MOVE", "move": move.uci()}
                            p2p_game.conn.sendall(json.dumps(packet).encode('utf-8'))
